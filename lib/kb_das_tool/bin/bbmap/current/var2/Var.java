...
public class Var implements Comparable<Var>, Serializable {
	...
	public static void main(String[] args){
		Timer t=new Timer();
		VarMap map=VcfLoader.loadVars(args[0], null);
...
//		a=b;
		...
		int cov=parseVcfInt(line, ";COV=", b);
		int mcov=parseVcfInt(line, ";MCOV=", b);
		int r1p=Tools.max(0, parseVcfInt(line, ";R1P=", b));
		int r2p=Tools.max(0, parseVcfInt(line, ";R2P=", b));
		int r1m=Tools.max(0, parseVcfInt(line, ";R1M=", b));
		int r2m=Tools.max(0, parseVcfInt(line, ";R2M=", b));
...
//		final Scaffold scaffold=scafMap.getScaffold(scafNum);
	...
	private static int parseVcfInt(byte[] line, String query, int start){
		int loc=Tools.indexOf(line, query, start);
		if(loc<0){return -1;}
		long current=0;
		int mult=1;
		if(loc>0){
		...
		return (int)Tools.min(Integer.MAX_VALUE, mult*current);
	}
...
//		bb.append(String.format(Locale.ROOT, "%.2f", toPhredScore(score))).append('\t');
		bb.appendFast(toPhredScore(score), 2).append('\t');
...
//		}
		...
		if(extendedText){
			...
			bb.appendFast(af, 4).append('\t');
			bb.appendFast(revisedAlleleFraction(af, readLengthAvg), 4).append('\t');
			bb.appendFast(strandRatio(), 4).append('\t');
			bb.appendFast(baseQAvg(), 2).append('\t');
			bb.appendFast(mapQAvg(), 2).append('\t');
			bb.appendFast(edistAvg(), 2).append('\t');
			bb.appendFast(identityAvg(), 2).append('\t');
			
			bb.appendFast(edistScore(), 4).append('\t');
			bb.appendFast(identityScore(), 4).append('\t');
			bb.appendFast(qualityScore(totalQualityAvg, totalMapqAvg), 4).append('\t');
			bb.appendFast(pairedScore(properPairRate, scafEndDist), 4).append('\t');
			bb.appendFast(biasScore(properPairRate, scafEndDist), 4).append('\t');
			bb.appendFast(coverageScore(ploidy, rarity, readLengthAvg), 4).append('\t');
			bb.appendFast(homopolymerScore(map), 4).append('\t');
			bb.appendFast(score, 4).append('\t');
		}
...
//		assert(vcfStart!=312207) : af+", "+raf+", "+readLengthAvg+", "+readlen();
...
//			final int covMinus=(Scaffold.trackStrand ? scaf.minusCoverage(this) : coverage/2);
			...
			bb.append("COV=").append(Tools.max(coverage, count)).append(';');
...
//			bb.append("SB=").append(String.format(Locale.ROOT, "%.4f", strandBias)).append(';');
			bb.append("AF=").appendFast(af,4).append(';');
			bb.append("RAF=").appendFast(raf,4).append(';');
			bb.append("SB=").appendFast(strandBias,4).append(';');
...
//		}
		{
			...
			bb.appendFast(raf,4);
			bb.append(':');
			bb.appendFast(strandBias,4);
			bb.append(':');
			
			bb.appendFast(score,2);
			bb.append(':');
			bb.append(pass ? "PASS" : "FAIL");
		}
...
//		System.err.println("E:"+new String(bases, pos-4, 9));
	...
	public double strandBiasScore(int scafEndDist){
		double x=eventProb(plusCount(), minusCount());
...
//		}
	...
	public double baseQAvg(){return baseQSum/(double)count();}
	public double mapQAvg(){return mapQSum/(double)count();}
	public double edistAvg(){return endDistSum/(double)count();}
	public double identityAvg(){return idSum/(double)count();}
	public double lengthAvg(){return lengthSum/(double)count();}
	public double properPairRate(){return properPairCount/(double)count();}
