...
public class VcfWriter {
	...
	public void writeVcfFile(final String fname){
		final FileFormat ff=FileFormat.testOutput(fname, FileFormat.VCF, null, true, true, false, true);
		writeFile(ff, true);
	}

	public void writeVarFile(final String fname){
		final FileFormat ff=FileFormat.testOutput(fname, FileFormat.VAR, null, true, true, false, true);
		writeFile(ff, false);
	}
	...
	private void writeFile(final FileFormat ff, final boolean vcfMode){
		assert(ff.ordered());
		...
		writeHeader(bsw, vcfMode);
		
		ArrayList<ProcessThread> alpt=spawnThreads(bsw, inq, vcfMode);
	...
	private void writeHeader(ByteStreamWriter bsw, boolean vcfMode){
	...
	private ArrayList<ProcessThread> spawnThreads(ByteStreamWriter bsw, final ArrayBlockingQueue<ListNum<Var>> inq, boolean vcfMode){
		...
		for(int i=0; i<threads; i++){
			alpt.add(new ProcessThread(i, bsw, inq, vcfMode));
		}
	...
	private class ProcessThread extends Thread{
		...
		public ProcessThread(int tid_, ByteStreamWriter bsw_, ArrayBlockingQueue<ListNum<Var>> inq_, boolean vcfMode_){
			tid=tid_;
			bsw=bsw_;
			inq=inq_;
			vcfMode=vcfMode_;
