...
//	}
	...
	public ArrayList<Sketch> loadSketches_MT(int mode, float samplerate, long reads, float minEntropy, String...fnames){
		...
		if(decomposedFnames.size()==1){return loadSketches(decomposedFnames.poll(), null, mode, 0, samplerate, reads, minEntropy, false);}
	...
	public ArrayList<Sketch> loadSketches(final String fname0, SketchMakerMini smm, 
...
//					allowMultithreadedFastq+", "+forceDisableMultithreadedFastq+", "+(mode==ONE_SKETCH);
			...
			if(ff.fastq() && allowMultithreadedFastq && !forceDisableMultithreadedFastq && mode==ONE_SKETCH &&
				...
				Sketch sketch=processReadsMT(fname0, mode, maxThreads, samplerate, reads, minEntropy, allowZeroSizeSketch);
	...
	private class LoadThread extends Thread{
		...
		public void run(){
			...
			for(String fname=queue.poll(); fname!=null; fname=queue.poll()){
				...
				try {
					temp=loadSketches(fname, smm, smm.mode, 1, samplerate, reads, smm.minEntropy(), false);
				} catch (Throwable e) {
