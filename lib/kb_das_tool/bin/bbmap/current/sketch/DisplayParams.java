...
//		printColors=true;
	...
	ByteBuilder queryHeader(Sketch sk){
		...
		if(sk.probCorrect<1 && sk.probCorrect>0){sb.append("\tQuality: ").appendFast(sk.probCorrect, 4);}
		if(sk.counts!=null){
			double d=Tools.averageDouble(sk.counts);
			sb.append("\tAvgCount: ").appendFast(d, 3);
			sb.append("\tDepth: ").appendFast(Tools.observedToActualCoverage(d), 3);
		}
...
//		sb.append(String.format(Locale.ROOT, "%.2f%%\t%.2f%%", 100*c.idMinDivisor(), 100*c.idMaxDivisor()));
		sb.appendFast(100*c.idMinDivisor(), 2).append('%').append('\t');
		sb.appendFast(100*c.idMaxDivisor(), 2).append('%');
...
//		if(printUContam){sb.append(String.format(Locale.ROOT, "\t%.2f%%", 100*c.uContamFraction()));}
		...
		if(printAni){sb.append('\t').appendFast(100*c.ani(aniFromWkid), 2).append('%');}
		if(printCompleteness){sb.append('\t').appendFast(100*c.completeness(), 2).append('%');}
		if(printContam){sb.append('\t').appendFast(100*c.contamFraction(), 2).append('%');}
		if(printContam2){sb.append('\t').appendFast(100*c.contam2Fraction(), 2).append('%');}
		if(printUContam){sb.append('\t').appendFast(100*c.uContamFraction(), 2).append('%');}
...
//		sb.append(rName).append(String.format(Locale.ROOT, "\t%.2f\t%d\t%d\t%d", 100*c.ani(aniFromWkid), sea, seb, ba));
		...
		sb.appendFast(100*c.ani(aniFromWkid), 2).append('\t');
