...
//		assert(false) : sketch+", "+parser.loglog;
		if(makeSketch){
			...
			tool=new SketchTool(sketchSize, 0, false, false);
	...
	void printSequenceResults(){
		...
		if(maxLen>0){println("MaxLen\t\t"+maxLen);}
		if(minLen<Integer.MAX_VALUE){println("MinLen\t\t"+minLen);}
		if(format!=FileFormat.FASTA && format!=FileFormat.UNKNOWN){println("QualOffset\t"+offset);}
		if(differs){}
...
//		if(loglog!=null){println("Cardinality\t"+loglog.cardinality());}
		...
		if(doMerge && pairsProcessed>0){
			...
			if(mergeFraction>0.02){
				...
				println("-AdapterReads\t"+String.format("%.2f%%", 100*adapterBaseFraction));
				println("-AdapterBases\t"+String.format("%.2f%%", 100*adapterReadFraction));
			}
		}
	}
...
//			println("-TrimmedAtQ30\t"+String.format("%.2f%%", trimhist[30]*trimMult));
		...
		if(qhistFile!=null && Tools.sum(qhist)>0){
			try {
				...
				printToFile(qhist, true, sb.toString(), qhistFile);
			} catch (Throwable e) {
...
//				}
		...
		private long[] qhist_T=new long[95];
	...
	private long[] qhist=new long[95];
