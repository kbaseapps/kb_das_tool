...
//		}
	...
	public static final TaxTree loadTaxTree(String taxTreeFile, String taxNameFile, String taxNodeFile, String taxMergedFile, PrintStream outstream, boolean hashNames){
		if(taxTreeFile!=null || taxNodeFile==null){
			TaxTree tree=sharedTree(taxTreeFile, hashNames, outstream);
	...
	public synchronized void hashNames(){
		assert(nameMap==null);
...
//	HashMap<TaxNode, Long> sizeMap;//TODO - will probably break serialization
	...
	private static TaxTree sharedTree(String fname, boolean hashNames, PrintStream outstream) {
		...
		if(sharedTree==null && fname!=null){
			synchronized(TaxTree.class){
				if(sharedTree==null){
					...
					Timer t=new Timer(outstream, false);
					setSharedTree(ReadWrite.read(TaxTree.class, fname, true), hashNames);
					t.stopAndPrint();
				}
			}
		}
	...
	private static synchronized void setSharedTree(TaxTree tree, boolean hashNames){
